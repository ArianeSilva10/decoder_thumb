...
int decod_thumb_file(char *name,FILE *in,FILE *out){ // utilizando operações BITWISE
...
/* funçao que decodifica apenas a instrução*/
void decod_thumb_instruction(char *instruction_hex){
    long int int_thumb; // Armazena a instrução em formato numérico
    char *ptr;
    int aux = 0;
    char transfer[100];

    // Converte a instrução hexadecimal para um número
    int_thumb = strtol(instruction_hex, &ptr, 16);

    // Decodifica a instrução
    aux = (int_thumb >> 12); // Pega os 4 bits mais significativos
    switch (aux) {
        case 0x0:
            if ((int_thumb >> 11) & 0x1) // Instrução LSR Ld, Lm, #immed5
                strcpy(transfer, "LSR ");
            else // Instrução LSL Ld, Lm, #immed5
                strcpy(transfer, "LSL ");
            // Buscando operandos
        ...
        case 0x1:
            if ((int_thumb >> 11) & 0x1) {
                if ((int_thumb >> 10) & 0x1) {
                    if ((int_thumb >> 10) & 0x1) // Instrução SUB Ld, Ln, #immed3
                        strcpy(transfer, "SUB ");
                    else // Instrução ADD Ld, Ln, #immed3
                        strcpy(transfer, "ADD ");

                    strcat(transfer, resister[(int_thumb) & 0x7]); // Ld
                    strcat(transfer, ",");
                    strcat(transfer, resister[(int_thumb >> 3) & 0x7]); // Ln
                    aux = (int_thumb >> 6) & 0x7; // immed3
                    strcat(transfer, ",#");
                    sprintf(_buffer, "%d", aux);
                    strcat(transfer, _buffer);
                    printf("%s\n", transfer);
                } else {
                    if ((int_thumb >> 10) & 0x1) // Instrução SUB Ld, Ln, Lm
                        strcpy(transfer, "SUB ");
                    else // Instrução ADD Ld, Ln, Lm
                        strcpy(transfer, "ADD ");
            ...
            } else { // Instrução ASR Ld, Lm, #immed5
                strcpy(transfer, "ASR ");
                strcat(transfer, resister[(int_thumb) & 0x7]); // Ld
                strcat(transfer, ",");
                strcat(transfer, resister[(int_thumb >> 3) & 0x7]); // Lm
        ...
        case 0x4:
            if((int_thumb >> 11) & 0x1){ // instruction LDR Ld,[pc,#immed*4]
                ...
                switch ((int_thumb >> 8) & 0x7)
                {
                case 0x0:// instruction AND/EOR/LSL/LSR Lm/Ls,Ld
                    if(((int_thumb >> 6) & 0x3) == 0x0)
                        strcpy(transfer,"AND ");
                    else if(((int_thumb >> 6) & 0x3) == 0x1)
                        strcpy(transfer,"EOR ");
                    else if(((int_thumb >> 6) & 0x3) == 0x2)
                        strcpy(transfer,"LSL ");
                    else
                        strcpy(transfer,"LSR ");
                ...
                case 0x4:
                    switch ((int_thumb>>6) & 0x3)
                    {
                    case 0x1:
                        strcpy(transfer,"ADD ");
                        strcat(transfer,resister_high[(int_thumb>>3)&0x7]); // Lm
                        strcat(transfer,",");
                        strcat(transfer,resister[(int_thumb)&0x7]); // Ld
                        printf("%s]\n",transfer);
                    break;
                    ...
                    case 0x2:
                        ...
                        strcat(transfer,resister[(int_thumb>>3)&0x7]); // Lm
                        strcat(transfer,",");
                        strcat(transfer,resister_high[(int_thumb)&0x7]); // Ld
                        printf("%s]\n",transfer);
                    break;
                ...
                case 0x7: // instruction BX/BLX Rm
                    if(((int_thumb >> 0x7) & 0x1) == 0x1)
                        strcpy(transfer,"BLX ");
                    else    
                        strcpy(transfer,"BX ");
                    //buscando operando
                    strcat(transfer,resister_all[(int_thumb>>3)&0xf]); // Lm
                    printf("%s]\n",transfer);
                break;
        ...
        case 0xb:
            if((int_thumb>>11)&0x1 == 1){
                if(((int_thumb>>9) & 0x3) == 0x1){
                    if(((int_thumb>>6) & 0x3) == 0x0){
                        strcpy(transfer,"REV ");
                        strcat(transfer,resister[(int_thumb >> 3) & 0x7]); //Lm
                        strcat(transfer,resister[(int_thumb) & 0x7]); //Ld
                ...
                }else if(((int_thumb>>9) & 0x3) == 0x2){
                    strcpy(transfer,"POP {");
                    int temp = 0;
                    for(int j = 7; j >= 0; j--){
                        if(((int_thumb>>(7-j)) & 0x1) == 1){
                            if(temp == 0){
                                temp == 1;
                                strcat(transfer,resister[7-j]);
                            }else{
                ...
                switch ((int_thumb>>9) & 0x3)
                {
                case 0x0:
                    if(((int_thumb>>7) & 0x1) == 0)
                        strcpy(transfer,"ADD sp,#");
                    else
                        strcpy(transfer,"SUB sp,#"); 

                    aux = (int_thumb) & 0x7f; // value immed7
                    sprintf(_buffer,"%d",(aux*4));
                    strcat(transfer,_buffer);
                    printf("%s\n",transfer);
                break;
                ...
                break;
                case 0x3:
                    if(((int_thumb>>5) & 0x7) == 0x2){
                        if(((int_thumb>>3) & 0x1) == 0x1)
                            strcpy(transfer,"SETEND BE");
                        else 
                            strcpy(transfer,"SETEND LE");
                    }else if(((int_thumb>>5) & 0x7) == 0x3){
                        if(((int_thumb>>4) & 0x1) == 0x1)
                            strcpy(transfer,"CPSID");
                        else 
                            strcpy(transfer,"CPSIE");
                    }else{
        ...
        case 0xc:
            if(((int_thumb>>11) & 0x1) == 1)
                ...
                strcpy(transfer,"STMIA ");
            strcat(transfer,resister[(int_thumb>>8)&0x7]); // Ln
            strcat(transfer,"!,{");
            int temp = 0;
            for(int j = 7; j >= 0; j--){
                if(((int_thumb>>(7-j)) & 0x1) == 1){
        ...
        break;
        case 0xd:
        if (((int_thumb >> 8) & 0xf) < 0xe) {
            strcpy(transfer, "B");
            strcat(transfer, condition[(int_thumb >> 8) & 0xf]); // Condição
            strcat(transfer, " #0x");
            int offset = (int_thumb & 0xff); // Offset de 8 bits (assinado)
            if ((int_thumb >> 7) & 0x1) offset = -offset; // Verifica se o offset é negativo
            offset = offset * 2 + 4; // Calcula o endereço relativo
            sprintf(_buffer, "%x", offset);
            strcat(transfer, _buffer);
            printf("%s\n", transfer);
        } else if (((int_thumb >> 8) & 0xf) == 0xe) {
            strcpy(transfer, "undefined");
            printf("%s\n", transfer);
        } else {
            strcpy(transfer, "SWI #");
            int swi_value = (int_thumb & 0xff); // Valor do SWI
            sprintf(_buffer, "%d", swi_value);
            strcat(transfer, _buffer);
            printf("%s\n", transfer);
        }
        break;
    ...
    case 0xe:
        if (((int_thumb >> 11) & 0x1) == 0x0) {
            strcpy(transfer, "B #0x");
            int offset = (int_thumb & 0x7ff); // Offset de 11 bits (assinado)
            if ((int_thumb >> 10) & 0x1) offset = -offset; // Verifica se o offset é negativo
            offset = offset * 2 + 4; // Calcula o endereço relativo
            sprintf(_buffer, "%x", offset);
            strcat(transfer, _buffer);
            printf("%s\n", transfer);
    ...
    default:
        printf("Instrução não suportada ou inválida.\n");
        break;
    }
}